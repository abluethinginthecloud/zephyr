...
static int tr230s_wait_until_ready(const struct device *dev)
{
	...
	ready_timeout = config->ready_timeout_ms == TR230S_READY_TIMEOUT_FOREVER_MS
			? K_FOREVER
			: K_MSEC(config->ready_timeout_ms);
...
#define TR230S_DEFINE(instance)                                                        \
	BUILD_ASSERT(DT_INST_STRING_UPPER_TOKEN(instance, mipi_mode) ==                  \
	...
	static const struct tr230s_config tr230s_config_##instance = {                   \
		...
		.ready_timeout_ms = (uint32_t)DT_INST_PROP(instance, ready_timeout_ms),    \
		.reset_pre_delay_ms =                                                      \
